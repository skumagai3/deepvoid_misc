...
def test_loss_functions():
    ...
    try:
        ...
        print(f"   PASS: SCCE_Class_Penalty: {tf.reduce_mean(loss1):.4f}")
    except Exception as e:
        print(f"   FAIL: SCCE_Class_Penalty failed: {e}")
    
    print("\n2. Testing SCCE_Balanced_Class_Penalty...")
    try:
        loss2 = nets.SCCE_Balanced_Class_Penalty(y_true, y_pred)
        print(f"   PASS: SCCE_Balanced_Class_Penalty: {tf.reduce_mean(loss2):.4f}")
    except Exception as e:
        print(f"   FAIL: SCCE_Balanced_Class_Penalty failed: {e}")
    
    # Test new improved functions
    print("\n3. Testing SCCE_Class_Penalty_Fixed...")
    try:
        loss3 = nets.SCCE_Class_Penalty_Fixed(y_true, y_pred)
        print(f"   PASS: SCCE_Class_Penalty_Fixed: {tf.reduce_mean(loss3):.4f}")
    except Exception as e:
        print(f"   FAIL: SCCE_Class_Penalty_Fixed failed: {e}")
    
    print("\n4. Testing SCCE_Proportion_Aware...")
    try:
        loss4 = nets.SCCE_Proportion_Aware(y_true, y_pred)
        print(f"   PASS: SCCE_Proportion_Aware: {tf.reduce_mean(loss4):.4f}")
    except Exception as e:
        print(f"   FAIL: SCCE_Proportion_Aware failed: {e}")
    
    print("\nPASS: All loss function tests completed!")
